...
def a_star_path(grid_X, grid_Y, score_map, start_idx, goal_idx):
...
def get_goal_from_score_map(score_map):
    flat_idx = np.argmax(score_map)
    return np.unravel_index(flat_idx, score_map.shape)

def move_one_step_a_star(score_map, grid_X, grid_Y, current_idx):
    goal_idx = get_goal_from_score_map(score_map)
    path = a_star_path(grid_X, grid_Y, score_map, current_idx, goal_idx)
    if len(path) >= 2:
        return path[1]
    return current_idx
...
def run_multi_robot_exploration_with_visualization(grid_X, grid_Y, robot_positions, robot_vmc_values, num_iterations=3, cell_size=cell_size):
    ...
    for step in range(num_iterations):
        ...
        for i in range(num_robots):
            ...
            new_idx = move_one_step_a_star(score_map, grid_X, grid_Y, curr_idx)
